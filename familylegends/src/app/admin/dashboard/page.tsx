...
export default function GuildDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [settingsRes, membersRes, economyRes, shopRes, warningsRes, statsRes, responderRes, notifyRes, healthRes, rrRes, backupRes, logsRes, giveawayRes] = await Promise.all([
        ...
        fetch(`/api/bot/members`),
        fetch(`/api/bot/economy`),
        fetch(`/api/bot/shop`),
        fetch(`/api/bot/warnings`).catch(() => null),
        fetch(`/api/bot/stats`).catch(() => null),
        fetch(`/api/bot/auto-responder`).catch(() => null),
        fetch(`/api/bot/notifications`).catch(() => null),
        fetch(`/api/bot/health`).catch(() => null),
        fetch(`/api/bot/reaction-roles`).catch(() => null),
        fetch(`/api/bot/backups`).catch(() => null),
        fetch(`/api/bot/logs?limit=20`).catch(() => null),
  ...
  const handleAddResponder = async () => {
    ...
    try {
      const res = await fetch(`/api/bot/auto-responder`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteResponder = async (trigger: string) => {
    try {
      const res = await fetch(`/api/bot/auto-responder?trigger=${encodeURIComponent(trigger)}`, { method: 'DELETE' });
  ...
  const handleDeleteStreamer = async (username: string, platform: string) => {
    try {
      const res = await fetch(`/api/bot/notifications?username=${username}&platform=${platform}`, { method: 'DELETE' });
      if (res.ok) { fetchData(); }
    } catch { toast({ variant: 'destructive', title: 'خطأ', description: 'فشل في الحذف' }); }
  };
  ...
  const handleSendEmbed = async () => {
    ...
    try {
      const res = await fetch(`/api/bot/embed`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleCreateBackup = async () => {
    ...
    try {
      const res = await fetch(`/api/bot/backups`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ creatorId: 'Dashboard Admin' }) });
  ...
  const handleSaveMember = async () => {
    ...
    try {
      const res = await fetch(`/api/bot/members/${editMember.userId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleResetMember = async () => {
    ...
    try {
      const res = await fetch(`/api/bot/members/${resetMember.userId}`, { method: 'DELETE' });
  ...
  const handleAwardBadge = async () => {
    ...
    try {
      const res = await fetch(`/api/bot/members/${selectedMember.userId}/badges`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            <Button asChild variant="ghost" size="icon" className="rounded-full">
              <Link href="/admin"><ArrowRight className="h-5 w-5" /></Link>
            </Button>
